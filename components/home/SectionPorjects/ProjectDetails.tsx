...
const ProjectDetails: FC<ProjectDetailsProps> = ({ details, handleCloseModal }) => {
  const { 
    id,
    name,
    meta,
        ...
        <div className='flex items-center'>
          {meta.map((m, index) => {
            if (index !== 0) {
              ...
              return (
                <span className='text-sm font-semibold text-slate-700 dark:text-slate-400' key={index}>{m}</span>
              )
            }
          })}
        </div>
